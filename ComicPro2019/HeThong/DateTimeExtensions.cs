...
namespace ComicPro2019
{
    public static class DateTimeExtensions
    {
        ...
        {
            DateTime outValue;
            if (DateTime.TryParse(val, out outValue))
            {
                if (outValue.ToString("dd/MM/yyyy") == "01/01/0001")
                {
                    return null;
                }
                else
        ...
        public static DataTable GetDateOfWeek(this DateTime date)
        {
            ...
            table.Rows.Add(2, "Tuần này", date.FirstDayOfWeek().ToString("dd/MM/yyyy"), date.LastDayOfWeek().ToString("dd/MM/yyyy"));
            table.Rows.Add(3, "Đầu tuần đến hiện tại", date.FirstDayOfWeek().ToString("dd/MM/yyyy"), date.Date.ToString("dd/MM/yyyy"));
            ...
            table.Rows.Add(13, "Tháng 2", date.ToString("01/02/yyyy"), Convert.ToDateTime(date.ToString("01/02/yyyy")).LastDayOfMonth().ToString("dd/MM/yyyy"));
            ...
            table.Rows.Add(18, "Tháng 7", date.ToString("01/07/yyyy"), date.ToString("31/07/yyyy")); table.Rows.Add(19, "Tháng 8", date.ToString("01/08/yyyy"), date.ToString("31/08/yyyy"));
            ...
            table.Rows.Add(28, "Hôm qua", date.AddDays(-1).ToString("dd/MM/yyyy"), date.AddDays(-1).ToString("dd/MM/yyyy"));
            table.Rows.Add(29, "Tuần trước", date.AddDays(-7).FirstDayOfWeek().ToString("dd/MM/yyyy"), date.AddDays(-7).LastDayOfWeek().ToString("dd/MM/yyyy"));
            table.Rows.Add(30, "Tháng trước", date.AddMonths(-1).FirstDayOfMonth().ToString("dd/MM/yyyy"), date.AddDays(-7).AddMonths(-1).LastDayOfMonth().ToString("dd/MM/yyyy"));
            table.Rows.Add(31, "Năm trước", date.AddYears(-1).ToString("01/01/yyyy"), date.AddYears(-1).ToString("01/12/yyyy"));
            table.Rows.Add(32, "Tuần sau", date.AddDays(7).FirstDayOfWeek().ToString("dd/MM/yyyy"), date.AddDays(7).LastDayOfWeek().ToString("dd/MM/yyyy"));
            table.Rows.Add(33, "Tháng sau", date.AddMonths(1).FirstDayOfMonth().ToString("dd/MM/yyyy"), date.AddDays(7).AddMonths(1).LastDayOfMonth().ToString("dd/MM/yyyy"));
            table.Rows.Add(34, "Năm sau", date.AddYears(1).ToString("01/01/yyyy"), date.AddYears(1).ToString("01/12/yyyy"));
            return table;
        }
